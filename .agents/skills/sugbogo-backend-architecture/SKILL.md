...
- For feature areas with multiple responsibilities, split services, serializers, and views into separate files by responsibility instead of placing unrelated classes in one large module.

Example:

```text
services/
├── jeepney_route_service.py
├── route_variant_service.py
├── transit_point_service.py
├── transit_transfer_service.py
└── __init__.py

serializers/
├── jeepney_route_serializers.py
├── route_variant_serializers.py
├── transit_point_serializers.py
├── transit_transfer_serializers.py
└── __init__.py

views/
├── jeepney_route_views.py
├── route_variant_views.py
├── transit_point_views.py
├── transit_transfer_views.py
└── __init__.py
```

- Keep closely related operations together within the same responsibility. For example, ordered route-point writes should remain with route-variant management when they are part of the same coordinated operation.

- Match the nearest existing multi-file feature structure in the repository rather than introducing catch-all service, serializer, or view modules.

- For backend testing, prioritize service and view/API tests. Do not create dedicated model or serializer test modules unless the existing repository already requires them for a specific feature.
...
services/
└── tests/
    ├── test_jeepney_route_service.py
    ├── test_route_variant_service.py
    ├── test_transit_point_service.py
    └── test_transit_transfer_service.py

views/
└── tests/
    ├── test_jeepney_route_views.py
    ├── test_route_variant_views.py
    ├── test_transit_point_views.py
    └── test_transit_transfer_views.py
```
...
- Prefer service tests under `services/tests/` and API/view tests under `views/tests/` for larger feature areas.

- Do not create dedicated model or serializer test modules unless a specific existing feature requires them. Cover model constraints or serializer behavior through the relevant service or API tests where practical.
...
- [ ] Split unrelated service, serializer, and view responsibilities into appropriate feature files when the feature is large enough to require multiple modules.

- [ ] Kept service and view/API tests in the repository's established test structure and avoided unnecessary dedicated model or serializer test modules.
