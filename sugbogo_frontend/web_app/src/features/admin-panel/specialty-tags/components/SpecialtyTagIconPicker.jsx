...
import SpecialtyTagIconPopover from "./SpecialtyTagIconPopover";
...
/**
 * Provides a searchable visual selector for specialty tag icons.
 *
 * Handles trigger state, viewport-aware popover positioning, selected icon
 * presentation, and interaction behavior.
 */
export default function SpecialtyTagIconPicker({
  value,
  onChange,
  error,
  required = false,
  ...
  const selected = SPECIALTY_TAG_ICONS.find((icon) => icon.value === value);
  ...
  // Keep the popover aligned while the viewport changes.
  useEffect(() => {
    if (!isOpen) {
      return undefined;
    }
  ...
  // Close the picker with Escape and restore trigger focus.
  useEffect(() => {
    if (!isOpen) {
      return undefined;
    }
