...
jest.mock("../../components/registration/location/ConfirmLocationSheet", () => ({
  __esModule: true,
  default: jest.fn(() => null),
}));
jest.mock("../../components/registration/location/LocationPickerHeader", () => ({
  __esModule: true,
  default: jest.fn(() => null),
}));
jest.mock("../../components/registration/location/BottomSelectionInfoSheet", () => ({
  __esModule: true,
  default: jest.fn(() => null),
}));
...
describe("LocationPickerScreen reverse-geocode ordering", () => {
  ...
  it("keeps a selected place when an older map request finishes later", async () => {
    ...
    (reverseGeocode as jest.Mock).mockReturnValue(
      new Promise((resolve) => { resolveMap = resolve; }),
    );
    const client = new QueryClient({
      defaultOptions: { queries: { retry: false, gcTime: Infinity } },
    });
    ...
    await act(async () => {
      header.mock.lastCall[0].onPlaceSelect({
        latitude: 10.32,
        longitude: 123.89,
