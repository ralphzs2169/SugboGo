...
class MerchantApplicationLocationSerializer(serializers.ModelSerializer):
    """Step 2 — location details."""

    province = serializers.CharField(
        source="MLOC_PROVINCE",
        error_messages={"blank": "Province is required.", "required": "Province is required."},
    )
    city = serializers.CharField(
        source="MLOC_CITY",
        error_messages={"blank": "City is required.", "required": "City is required."},
    )
    barangay = serializers.CharField(
        source="MLOC_BARANGAY",
        error_messages={"blank": "Barangay is required.", "required": "Barangay is required."},
    )
    ...
    unit = serializers.CharField(
        source="MLOC_UNIT", required=False, allow_blank=True, allow_null=True
    )
...
class MerchantApplicationLandmarkSerializer(serializers.ModelSerializer):
    """Step 3 — one landmark per record; frontend calls this once per landmark."""

    name = serializers.CharField(
        source="MLMK_NAME",
        error_messages={"blank": "Landmark name is required.", "required": "Landmark name is required."},
    )
    address = serializers.CharField(
        source="MLMK_ADDRESS",
        error_messages={"blank": "Landmark address is required.", "required": "Landmark address is required."},
    )
    latitude = serializers.FloatField(write_only=True)
    longitude = serializers.FloatField(write_only=True)
    ...
    place_id = serializers.CharField(
        source="MLMK_PLACE_ID", required=False, allow_blank=True, allow_null=True
    )
...
class MerchantApplicationLandmarkReadSerializer(serializers.ModelSerializer):
    """Nested, read-only view of one landmark."""
    ...
    class Meta:
        model = MerchantApplicationLandmark
        fields = ("id", "name", "address", "latitude", "longitude", "source", "place_id")
...
class MerchantApplicationLocationReadSerializer(serializers.ModelSerializer):
    """Nested, read-only view of Location, including its landmarks."""
    ...
    street_address = serializers.CharField(source="MLOC_STREET_ADDRESS", read_only=True)
    unit = serializers.CharField(source="MLOC_UNIT", read_only=True)
    latitude = serializers.SerializerMethodField()
    longitude = serializers.SerializerMethodField()
    landmarks = MerchantApplicationLandmarkReadSerializer(many=True, read_only=True)
